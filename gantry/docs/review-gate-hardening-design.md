...
**Planner (`agents/phase-planner.md`).** Every phase that introduces a new public capability (an exported function, system, command, bus event, or user-facing control) carries a one-line wiring declaration in its phase entry:
...
This forces the plan to own the wiring (the root cause of finding 1). One line per relevant phase, not a new section.

**Reviewer (`agents/phase-reviewer.md`), under Code discipline.** For each new exported function, system, command, bus event, or user-facing capability in the diff, grep for a live (non-test) caller. The verdict rule targets **silent** dead code only:
...
**Round cap (`skills/gantry/SKILL.md`, `commands/review.md`).** Each review loop caps at **3** rounds: the primary's FAIL -> fix -> re-review loop, and (when configured) the adversary's, are each capped separately. If either reaches 3 rounds without a clean verdict, the orchestrator stops and escalates to the human, informed: it shows the finding trajectory (shrinking = a deep phase worth continuing, recurring = stuck) and the three options (keep iterating, amend the plan, overrule with a logged reason). It never auto-passes; a real defect cannot slip through the cap.
...
The v1 units are prompt/doc-only, so their phases need a real check beyond "the text reads right": add assertions to `test/consistency.test.js` (the repo's `node --test` suite) confirming the Reachability and real-input checks appear in `phase-reviewer.md`, the wiring declaration in `phase-planner.md`, the real-input rule in `implementer.md`, and the round cap in the orchestrator skill. Unit 4's routing change gets real unit tests in `test/role-core.test.js` (adversary on a reviewer resolves; on implementer/planner is ignored; malformed fails safe; default config yields no adversary) and `test/role.test.js` (`resolve` shows the adversary line; `run --adversary` builds the second invocation).

## Contracts touched

- **`agents/phase-reviewer.md`**: reachability (U1), real-input (U2), scope calibration (U3). Prompt-only.
- **`agents/phase-planner.md`**: per-phase wiring declaration (U1). Prompt-only.
- **`agents/implementer.md`**: real-input verification (U2). Prompt-only.
- **`skills/gantry/SKILL.md`, `commands/run.md`, `commands/review.md`**: the round cap (U3) and the adversary final-pass step (U4). Orchestrator doc edits.
- **`.gantry/models.json` schema (additive)**: optional `adversary` on a reviewer role; documented in **`commands/models.md`** with the opus-primary + codex-adversary example.
- **`scripts/role-core.js`**: additive `adversary` field on `resolveRole`'s return; all existing fields unchanged. Pure module, no IO. New `test/role-core.test.js` cases.
- **`scripts/role.js`**: `resolve` `ADVERSARY:` line, `run --adversary` flag, reusing the existing `cli`/stdin/Windows-shim path. New `test/role.test.js` cases.
- **`test/consistency.test.js`**: section-presence assertions for the prompt changes (U5).
...
- **Adversary backend not installed on this machine** (codex absent): fail open - warn and let the primary-passed diff proceed to the gate. A missing optional second reviewer must never block a commit. (Mirrors role.js's ENOENT handling.)
...
All resolved in a grill (2026-06-14) and baked into the units above; recorded so the reviewer does not re-open them:
...
- Round cap: **N=3 per loop**, simple count, informed escalation, never auto-passes.
