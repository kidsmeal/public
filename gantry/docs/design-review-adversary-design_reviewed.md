...
After the primary `design-reviewer` reaches a clean result (all `[NEEDS USER DECISION]` markers resolved and the reviewed doc's summary reads "Ready for phase planning: yes"), and before the gate to Stage 2 opens, run `node ${CLAUDE_PLUGIN_ROOT}/scripts/role.js resolve design-reviewer` and inspect the `ADVERSARY:` line:
...
`/gantry:design` runs the `design-reviewer` standalone, outside the full orchestrator. It mirrors the same adversary pass so driving the design gate by hand gets the same second opinion. After the primary writes `<draft>_reviewed.md` and its summary reads "Ready for phase planning: yes", run `role.js resolve design-reviewer`; if an adversary is configured, run `role.js run design-reviewer --adversary -- <reviewed-doc> [rubric]`, relay both summaries, and apply the same resolve-and-re-run guidance and the same fail-open rule. Kept proportionate to a command doc (terser than the SKILL.md stage), but the outcomes and the fail-open behavior are identical.

Note for planning: when the primary `design-reviewer` runs on an `external` backend, it writes `<draft>_reviewed.md` itself (per design.md's existing `DISPATCH: external` branch). The adversary run is always read-only on the reviewed doc — it produces a summary, it does not write the doc — so the adversary pass composes with either dispatch mode of the primary without a write-ordering conflict.

### Unit 3: adversary prompt note, role-appropriate (`plugins/gantry/scripts/role.js`)

`role.js` appends a one-line note to the adversary's prompt (currently `role.js:219-221`): "the **diff** above has already passed the primary reviewer." That wording is correct for the `phase-reviewer` adversary but wrong for the `design-reviewer` adversary, whose input is a design doc, not a diff. Make the note role-appropriate: for a `design-reviewer` adversary it should read in terms of the design doc having already passed the primary design review; for `phase-reviewer` it keeps the diff wording. This is the one code edit in the feature. The note is branched on the `role` argument already in scope in `cmdRun` (the same `role === "design-reviewer"` check the rest of the unit relies on); `role-core.js` and the `resolve`/`run --adversary`/`show` surfaces already handle `design-reviewer` and are untouched.

### Unit 4: test artifact (`test/consistency.test.js`, `test/role.test.js`)

- `consistency.test.js`:
  - **New assertions:** SKILL.md Stage 1 and design.md carry the adversary step, pinning literal anchors (`design-reviewer`, `--adversary`, `ADVERSARY`, the fail-open phrasing, "both clean"/the both-clean gate condition), mirroring how the Phase 6 anchors already pin the `phase-reviewer` adversary in Stage 3.
  - **Existing assertion to flip (required, not optional):** the shipped test `"models.md states the design-reviewer adversary is parsed and shown but dormant"` (currently `consistency.test.js:402-414`) asserts `models.md` matches `/dormant|v2|reserved/i`. Once `models.md`'s prose changes to the active design-gate behavior (see Contracts touched), that assertion will fail. Replace it with the inverse: assert `models.md` no longer claims the `design-reviewer` adversary "does not fire" / is "reserved for v2" / "dormant", so the dropped caveat cannot silently return. This is a phase task, not a side effect: the consistency suite must be edited in the same phase that edits `models.md`, or the suite goes red.

- `role.test.js`: assert the adversary prompt note is role-appropriate — a `design-reviewer --adversary` run does not emit diff-specific wording, and a `phase-reviewer --adversary` run keeps it. Extend the existing `mkdtemp` + `GANTRY_PROJECT_DIR` spawn pattern. Note: the shipped test `"show: dormant design-reviewer adversary is parsed and shown"` (`role.test.js:235-260`) still passes unchanged (its behavior — `show` lists a configured `design-reviewer` adversary — is unaffected by this feature); only its comment/description text mentions "dormant (v2)", which becomes stale. Refresh that wording so the test name does not assert a caveat the feature removes.
...
- **`/gantry:run` command summary (`plugins/gantry/commands/run.md`).** `run.md` drives the full pipeline by following the gantry skill, and it already spells out the `phase-reviewer` adversary final pass in its Stage 3 summary (item 3). Its Stage 1 summary (item 1) names the design-review gate but not a design adversary. To keep the command doc consistent with SKILL.md Stage 1 after this change, `run.md` item 1 gains a one-line mention of the design adversary pass (resolve `design-reviewer`, run `--adversary` on the clean reviewed doc when configured; fail-open; gate opens only when both primary and adversary are clean or the human overrules). Prose-only, additive, no behavior of its own beyond what the skill defines.
...
- **`.gantry/models.json` schema:** no change. The `adversary` field already exists and is already valid on `design-reviewer` (v1.5 resolves it). Only the prose in `models.md` describing its behavior changes — from "parsed and shown but dormant / reserved for v2" to the active design-gate behavior. This `models.md` edit is what makes the existing `consistency.test.js:402-414` assertion fail; that test is flipped in the same phase (Unit 4).
- **No change** to the `phase-reviewer` adversary, `role-core.js`, the routing/dispatch code, or `review.md`.
...
- **Primary on an external backend (writes the `_reviewed` doc itself):** the adversary run reads the reviewed doc and writes nothing, so it composes with the external primary's write without a conflict (Unit 2).
...
- `review.md` (it governs the phase build/review gate, not the design gate; the design gate never touches it).
