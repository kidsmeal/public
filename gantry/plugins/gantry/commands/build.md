...
This command is also the fix-relay path for the adversary reviewer: when the adversary (run via `role.js run phase-reviewer --adversary`) returns a FAIL or fix-now notes, the orchestrator widens the sentinel with `sentinel.js add-files` for the adversary's cited paths and then re-invokes `/gantry:build <plan> <phase>` with the adversary's Required fixes as the scoped input. Build itself is unchanged - the same implementer spawn as a normal fix pass. After the fix the adversary (not the primary) is re-run.
