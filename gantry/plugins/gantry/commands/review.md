...
- On **FAIL**: do not commit. Before re-spawning the implementer for the fix pass, run: `node ${CLAUDE_PLUGIN_ROOT}/scripts/sentinel.js add-files <space-separated list of the reviewer's cited file paths>` (this widens the active sentinel so the fix pass can touch exactly those files). Then send the Required fixes back to the implementer via `/gantry:build $1 $2`, and re-review. Repeat up to 2 cycles; if still failing, stop and hand it to me.
