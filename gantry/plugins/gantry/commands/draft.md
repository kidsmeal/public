...
Do this by following the **design-plan-creator** skill: load the project's conventions and the area the feature touches, then **grill me** through the open decisions one branch at a time - first the why (what the user or player should feel or get, in plain terms, no mechanics), then data/model, scope, behavior, contracts, edge cases - resolving each against the real codebase before writing anything. Grill first, write once. Only once the decisions are settled, write a structured design doc (Intent/the why, Problem, Design, Contracts touched, Edge cases, Out of scope, Open questions).
