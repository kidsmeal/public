...
2. If no convention file was found, tell me the agents will fall back to matching the surrounding code, and offer to help write a short `CONVENTIONS.md` later (do not write it now). Separately, offer a short `docs/ARCHITECTURE.md`: a coarse map of the main modules plus the gotchas grep can't surface (the module that looks dead but isn't, the load-bearing implicit contract, the surprising vocabulary). The planner reads it before decomposing a feature, so it grounds the plan instead of making the planner re-derive the shape cold. Keep it to landmarks, twenty lines not two hundred. Offer it, do not write it now, and never generate an exhaustive file inventory: coarse and true, or skip it.
